#!/usr/bin/env bash
#
# libriscv/mayhem/build.sh — build libriscv's in-tree fuzz harness (upstream fuzz/fuzz.cpp) as
# sanitized libFuzzer targets plus standalone (non-fuzzer) reproducers.
#
...
FUZZ_SRC="$SRC/fuzz/fuzz.cpp"
[ -f "$FUZZ_SRC" ] || { echo "ERROR: $FUZZ_SRC missing" >&2; exit 1; }

# ── 1) Build the libriscv static library WITH sanitizers (the fuzzed code is instrumented) ────────
...
INCLUDES=(-I "$SRC/lib" -I "$SRC/build")
