...
// Mayhem overlay: return int (not void) so libFuzzer propagates the return value correctly.
// A void return causes libFuzzer to treat every input as "interesting" with garbage coverage
// data, which manifests as 0 new edges and stalled runs. The fix is additive: this file lives
// in mayhem/ and is compiled instead of the upstream fuzz/fuzz.cpp (see mayhem/build.sh).
extern "C"
